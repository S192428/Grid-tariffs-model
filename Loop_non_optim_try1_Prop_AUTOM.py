...
for bl in range(len(Design_test)):
    # ----------------------------------------------------------------------------
    ...
    for i in range(Nbr_Cons):
        Consumers.loc[i,'Heat Demand'] = 'P' + str(2)
    ...
    Param_prev = pd.DataFrame(0,index=['Values'], columns=['NM','NNM','VNP','CNP','FNP','Vol_per','Capa_per'])
    ...
    name = 'D:/DTU/2A/MSC Thesis/Model/New/'+'BC' + str(BC) + '_al' + str(Param_prev.loc['Values','Vol_per']) + '_b' + str(Param_prev.loc['Values','Capa_per']) + '_NM' + str(Param_prev.loc['Values','NM']) + '_Nbr' + str(Nbr_Cons) + '_ProProp' + str(Pro_Pen) + '_SC' + str(Sunk_Costs) + '_CO2C' + str(Carbon_price) +'.xlsx'
